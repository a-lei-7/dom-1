...
window.dom = {
  create: function create(string) {
    var container = document.createElement("template"); //template ： html 中可以容纳所有元素的标签
  ...
  find: function find(selector, scope) {
    ...
    return (scope || document).querySelectorAll(selector); //如果有 scope 就在scope 内寻找，如果没有就在总的 document 内寻找
    //返回值是数组，所以要加上 下标[]
  },
  ...
  index: function index(node) {
    var list = dom.children(node.parentNode); // 用 dom.children 取得 node 的父亲的所有 children ，然后利用 for 循环和 if ，判断 list[i] === node,是的话就停止循环，返回 i 

    var i; //在 for 的外面声明 i ，以便在 for 外面返回 i
